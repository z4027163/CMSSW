...
void PlotStackzonly::plotmZ(std::string histlabel){
...
//   Color_t h400Color = kRed;
  ...
  if (histlabel.find("hMZ1_5")<10){
    hframe= new TH2F("hframe","hframe",60,60.,120.,500,1.0,10000000.);// mZ1 
    hframe2= new TH2F("hframe2","hframe2",600, 60., 120., 1000, 0.5, 2.);// mZ1 
  }

  if (histlabel.find("hPtZ1_5")<10){
    hframe= new TH2F("hframe","hframe",100,0.,200.,500,1.0,10000000.);// mZ1 
  ...
  if (histlabel.find("Mbb_6")<10){
    ...
    hframe2= new TH2F("hframe2","hframe2",80, 20., 400., 1000, 0.5, 2.);// mZ1 
  }
  ...
  if (histlabel.find("bdiscr_5_lead")<10){
    ...
    hframe2= new TH2F("hframe2","hframe2",20, 0., 1., 1000, 0.5, 2.);// mZ1 
  }
  ...
  if (histlabel.find("hPFMET_8")<10){
    ...
    hframe2= new TH2F("hframe2","hframe2",300, 0.,300., 1000, 0.5, 2.);// PFMET
  }
  ...
  for ( int datasetId=Vdatasetnamebkg.size()-1; datasetId >=0; datasetId--){  
	...
	//hfourlepbestmass_4l_afterSel_new_new->Scale(double(4710.*3048.*11974371./80767910.)*double(hfourlepbestmass_4l_afterSel_new_new->GetEntries()/11974371./hfourlepbestmass_4l_afterSel_new_new->GetEntries()));
	 hfourlepbestmass_4l_afterSel_new_new->Scale(double(6025.2*759.*nnn/946855.));//*double(hfourlepbestmass_4l_afterSel_new_new->GetEntries()/12138430./hfourlepbestmass_4l_afterSel_new_new->GetEntries()));
	...
	if(datasetnamebkg.find("TT_TuneCUETP8M1") < 200){
          hfourlepbestmass_4l_afterSel_new_new->Scale(double(831.76*759.*nnn/1980000.)); 
	  ...
	  char temp[328];
	  sprintf(temp,"%s",histosdir.c_str());
	  if((datasetnamebkg.find(temp) < 200|| datasetnamebkg.find(whichsample) < 200) && hfourlepbestmass_4l_afterSel_new_TT->GetEntries()>0. ) legend->AddEntry(hfourlepbestmass_4l_afterSel_new_new,Vlabelbkg.at(datasetId).c_str(), "F");
	  //hfourlepbestmass_4l_afterSel_new_new->Draw("sameP");
	}      
...
void PlotStackzonly::setSamplesNamesZ()
{
...
void PlotStackzonly::printnumbers(char name[400], TH1F *h){
