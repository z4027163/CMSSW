...
rootlib=/cvmfs/cms.cern.ch/slc6_amd64_gcc630/external/boost/1.63.0-mlhled2
...
if [ $1 == "llbb" ]; then
    ...
    g++ -I $ROOTSYS/include -I $ROOFITSYS/include -I ${rootlib}/include -I ${melaincdir}  -I ${cmsswincdir} compilereference_list.C HZZ4LeptonsAnalysis_llbb.C -I roccor `root-config --glibs` `root-config --libs` `root-config --cflags` -L $ROOFITSYS/lib  -lRooFit -lRooFitCore -L ${melalibdir} -L ${cmsswlibdir} -lCondFormatsBTauObjects -lCondToolsBTau -I $melaincdir/CondFormats -o RunReference_llbb

elif [ $1 == "btag" ]; then
    echo "Compiling $1 macros" 
    g++ -I $ROOTSYS/include -I $ROOFITSYS/include -I ${melaincdir} -I ${rootlib}/include -I ${cmsswincdir} compilereference_list.C bjet_efficiency.C -I roccor `root-config --glibs` `root-config --libs` `root-config --cflags` -L $ROOFITSYS/lib  -lRooFit -lRooFitCore -L ${melalibdir} -L ${cmsswlibdir}  -lCondFormatsJetMETObjects   -lCondFormatsBTauObjects -lCondToolsBTau  -o RunReference_btag
