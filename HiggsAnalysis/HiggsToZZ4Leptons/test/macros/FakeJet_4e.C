...
// setup calibration + reader
  ...
  TFile *j_scale_factors3_p1 = new TFile("btag/fake_JetHT_BF.root");
  TH2F *j_eff_p1 = (TH2F*)gDirectory->Get("Jet_9");
   ...
   TH1F * M4l = new TH1F("M4l","invariant mass of 4l after step 6",500,0,500);
...
//	  Zcandvector.push_back(*Z);	  
      ...
      int jetfail2[100]={0};
      for(int i = 0; i < Ne_good; ++i){
        if (fabs(RECOELE_PFX_rho_new[iLe[i]])>=0.35) continue; //
        if(iLe[i]==Zcandvector.at(0).ilept1||iLe[i]==Zcandvector.at(0).ilept2) continue;
...
//	  if (fabs(RECOELE_SIP[iLe[j]])>=4.) continue;
       ...
       bool goodjet = RECO_PFJET_NHF[i] < 0.90 &&
          ...
          if(RECO_PFJET_PT[j]>10. && fabs(RECO_PFJET_ETA[j])<2.5 && goodjet==1){
...
//          if(massZ<60||massZ>120) continue;
	...
	continue; 
      ...
      if(debug) cout << "Z2 number=" << Z2candvector.size() << endl;
...
//qier metcut
     if(metcut&&RECO_CORMETMUONS>40&&RECO_CORMETMUONS<80) continue;
      // **** Step 4:
       // a) 4 leptons
      // b) pair #2
      // c) highest pt
...
//de_trig matching(qier)
	...
	sortedpT[i]=tmp_max_pt;
      ...
      if(massZ1<40||massZ2<40) continue;
      //4mu selection qier  rev or rev2
 ...
 */
      ...
      M4l->Fill((Z1P4+Z2P4).M(),newweight);
