...
//  TH2F *mu_scale_factors_id_p1 = (TH2F*)gDirectory->Get("MC_NUM_MediumID2016_DEN_genTracks_PAR_pt_eta/abseta_pt_ratio");
  ...
  TH2F *j_eff_p1 = (TH2F*)gDirectory->Get("Bot_9");
  ...
  TH1F *b_fk_p4 = (TH1F*)gDirectory->Get("f_ratio_j");
   ...
   TH1F * hPtBot_9 = new TH1F("hPtBot_9", "Pt of (no ID)jet after selection step 5", 300 ,  0 , 600 );
   hPtBot_9->SetXTitle("pt_jet  (GeV)");

   TH1F * hYBot_9 = new TH1F("hEtaBot_9", "Y of (no ID)jet after selection step 5", 500 , -5. , 5. );
   hYBot_9->SetXTitle("Y of jet");

   TH1F * hPtMu_8 = new TH1F("hPtMu_8", "Pt of ele after selection step 5", 300 ,  0 , 600 );

   TH1F * hEtaMu_8 = new TH1F("hEtaMu_8", "Y of ele jet after selection step 5", 500 , -5. , 5. );



   TH1F * hPtBot_8 = new TH1F("hPtBot_8", "Pt of jet after selection step 5", 300 ,  0 , 600 );
   hPtBot_8->SetXTitle("pt_jet  (GeV)");
   ...
   TH1F * hYBot_8 = new TH1F("hEtaBot_8", "Y of jet after selection step 5", 500 , -5. , 5. );
   hYBot_8->SetXTitle("Y of Jet8");
...
//Bottom plot
   ...
   TH1F * Mmm_8 = new TH1F("Mmm_8", "Mass jets fake ee", 200, -0.5, 499.5 );
   Mmm_8->SetXTitle("Mass jets");

   TH1F * Mmm_7 = new TH1F("Mmm_7", "Mass mm ", 200, -0.5, 499.5 );
   Mmm_7->SetXTitle("Mass jets");

   TH1F * Mmm_9 = new TH1F("Mmm_9", "Mass mm ", 200, -0.5, 499.5 );
   Mmm_9->SetXTitle("Mass jets");
...
//	if (RECOELE_PFX_rho_new[iLe[i]]>=0.20) continue;
	...
	if (flagFSR==1){
     ...
     int njets_pass=0;
     bool lead_jet=false;
     TLorentzVector JET0,JET1,JET2,MU0,MU1,MU2;
     int jet1=-999,jet2=-999,mu1=-999,mu2=-999;      
     int jetfail[100];
     ...
     MU0.SetPtEtaPhiE(RECOMU_PT[iL[0]],RECOMU_ETA[iL[0]],RECOMU_PHI[iL[0]],RECOMU_PT[iL[0]]*TMath::CosH(RECOMU_ETA[iL[0]]));
     if(RECOMU_PT[iL[0]]<30) continue;
     if(!RECOMU_sm_MuHLTMatch[iL[0]]) continue;
     if (RECOMU_PFX_dB_new[iL[0]]>=0.20) continue;
     ...
     for(int i=0;i<RECO_PFJET_N;i++){
       ...
       if(RECO_PFJET_PT[i]>10. && fabs(RECO_PFJET_ETA[i])<2.4/*&&cSV_BTagJet_DISCR[i]> 0.5426*/){
	 ...
	 if (jetfail[i]==0){
	   ...
	   njets_pass++;
           ...
           if(cSV_BTagJet_DISCR[i]> 0.5426){
             hPtBot_8->Fill(RECO_PFJET_PT[i],newweight);
             hYBot_8->Fill(RECO_PFJET_ETA[i],newweight);
           }
             hPtJet_8->Fill(RECO_PFJET_PT[i],newweight);

           int binx = b_fk_p1->GetXaxis()->FindBin(RECO_PFJET_PT[i]);
           if(binx>=11) binx=11;

           double scale_fkj=b_fk_p1->GetBinContent(binx);
           double scale_fkj_j=b_fk_p3->GetBinContent(binx);
           double u3 = gRandom->Rndm();
           int bins= b_fk_p2->GetNbinsX();
           double ptx=5.0/Pt;
           int bxmin=b_fk_p2->GetXaxis()->FindBin(ptx);
 
           int bin_eff=b_eff_p1->GetXaxis()->FindBin(RECO_PFJET_PT[i]);
           double b_eff=b_eff_p1->GetBinContent(bin_eff); 
           scale_fkj=scale_fkj/(1-b_fk_p2->GetBinContent(bxmin))/b_eff;
           scale_fkj_j=scale_fkj_j/(1-b_fk_p4->GetBinContent(bxmin));
           double pscale=1;
           double pscale_j=1;
         for(int kk=1;kk<bins;kk++){
           if(u3>=b_fk_p2->GetBinContent(kk)&&u3<b_fk_p2->GetBinContent(kk+1))                   pscale=b_fk_p2->GetBinCenter(kk);
           if(u3>=b_fk_p4->GetBinContent(kk)&&u3<b_fk_p4->GetBinContent(kk+1))
              pscale_j=b_fk_p4->GetBinCenter(kk);
         }
         Pt=Pt*pscale;
         Pt_j=RECO_PFJET_PT[i]*pscale_j;
//         cout << "f= " <<u3 << "  scale=" << pscale << endl;
           JET0.SetPtEtaPhiE(Pt,RECO_PFJET_ETA[i],RECO_PFJET_PHI[i],RECO_PFJET_ET[i]*TMath::CosH(RECO_PFJET_ETA[i])); 
           JET1.SetPtEtaPhiE(Pt_j,RECO_PFJET_ETA[i],RECO_PFJET_PHI[i],RECO_PFJET_ET[i]*TMath::CosH(RECO_PFJET_ETA[i]));
           double Mmm=(MU0+JET0).M();
           double Mmm_j=(MU0+JET1).M();
           if(cSV_BTagJet_DISCR[i]> 0.5426&&Pt>10){
             Mmm_8->Fill(Mmm,newweight*scale_fkj);          
             hMZ1_5->Fill(Mmm,newweight*scale_fkj);
             hPtBot_9->Fill(Pt,newweight*scale_fkj);
             hYBot_9->Fill(RECO_PFJET_ETA[i],newweight*scale_fkj);
           }
           if(Pt_j>10){
           Mmm_9->Fill(Mmm_j,newweight*scale_fkj_j);
           hPtJet_9->Fill(Pt_j,newweight*scale_fkj_j);
           }
	 }
     ...
     for(int i = 1; i < N_good; ++i){
          if (RECOMU_PFX_dB_new[iL[i]]>=0.20) continue;
          if (RECOMU_PT[iL[i]]<10) continue;
          double deltaR = sqrt( pow( DELTAPHI( RECOMU_PHI[iL[0]] , RECOMU_PHI[iL[i]] ),2) + pow(RECOMU_ETA[iL[0]] - RECOMU_ETA[iL[i]],2) );
          if(deltaR<0.3) continue;
          nmu++;

            if(RECOMU_CHARGE[iL[0]]!=RECOMU_CHARGE[iL[i]]){
             hPtMu_8->Fill(RECOMU_PT[ iL[i] ],newweight);
             hEtaMu_8->Fill(RECOMU_ETA[ iL[i] ],newweight);
            }
           if (nmu==1){
             mu2=iL[i];
             MU2.SetPtEtaPhiE(RECOMU_PT[iL[i]],RECOMU_ETA[iL[i]],RECOMU_PHI[iL[i]],RECOMU_PT[iL[i]]*TMath::CosH(RECOMU_ETA[iL[i]]));
             if(RECOMU_CHARGE[iL[0]]==RECOMU_CHARGE[iL[i]]) sscharge=true;
           }
     }
     if(nmu>=1){
       double Mmm=(MU0+MU2).M();
       Mmm_7->Fill(Mmm,newweight); 
     }
