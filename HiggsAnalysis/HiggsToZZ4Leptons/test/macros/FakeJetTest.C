...
//	output_txt  << leptformat << endl;
	...
	if (flagFSR==1){
     ...
     for(int i=0;i<RECO_PFJET_N;i++){
       ...
       if(RECO_PFJET_PT[i]>20. && fabs(RECO_PFJET_ETA[i])<2.4 /*&& goodjet==1*/){
	...
	//   if (RECOELE_PFX_rho_new[iLe[ele]]>=0.20) continue;
     ...
     for(int i = 0; i < Ne_good; ++i){
          ...
          hEtaEle_8->Fill(RECOELE_ETA[ iLe[i] ],newweight);
           ...
           if (nele==1){
             ele1=iLe[i];
             ELE1.SetPtEtaPhiE(RECO_PFJET_PT[iLe[i]],RECO_PFJET_ETA[iLe[i]],RECO_PFJET_PHI[iLe[i]],RECO_PFJET_ET[iLe[i]]*TMath::CosH(RECO_PFJET_ETA[iLe[i]]));
           }
           if (nele==2){
             ele2=iLe[i];
             ELE2.SetPtEtaPhiE(RECO_PFJET_PT[iLe[i]],RECO_PFJET_ETA[iLe[i]],RECO_PFJET_PHI[iLe[i]],RECO_PFJET_ET[iLe[i]]*TMath::CosH(RECO_PFJET_ETA[iLe[i]]));
           }
     }
     if(nele>=2){
       double Mee=(ELE1+ELE2).M();
     ...
     int binyj1 = j_eff_p1->GetYaxis()->FindBin(RECO_PFJET_PT[jet1]);
     int binxj1 = j_eff_p1->GetXaxis()->FindBin(RECO_PFJET_ETA[jet1]);
     int binyj2 = j_eff_p1->GetYaxis()->FindBin(RECO_PFJET_PT[jet2]);
     int binxj2 = j_eff_p1->GetXaxis()->FindBin(RECO_PFJET_ETA[jet2]);

     double scale_fkj_1=j_eff_p1->GetBinContent(binxj1,binyj1);
     double scale_fkj_2=j_eff_p1->GetBinContent(binxj2,binyj2);

     Mee_8->Fill(Mjj,newweight*scale_fkj_1*scale_fkj_2);
     Mjj_6->Fill(Mjj,newweight);

     hMZ1_5->Fill( Mjj,newweight );
