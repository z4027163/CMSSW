...
void HZZ4LeptonsAnalysis::Loop(Char_t *output)
{
   ...
   bool metcut=false;
   int channel=3; // 4mu2b 1, 2e2mu2b 2, 4e2b 3
   int rev=1;     // rev 1, rev2 2
   if(era=="BF"){
...
// setup calibration + reader
  ...
  TH1F *pujet_p1 = (TH1F*)gDirectory->Get("sf_pujet");
  TH1F *pujet_p2 = (TH1F*)gDirectory->Get("sf_pubjet");
   ...
   TH1F * dR_l_1 = new TH1F("dR_l_1", "dR of loose mu to closest jet", 50 , 0. , 5. );
   TH1F * dR_l_2 = new TH1F("dR_l_2", "dR of loose mu to closest jet", 50 , 0. , 5. );
   TH1F * dR_l_3 = new TH1F("dR_l_3", "dR of loose mu to closest jet", 50 , 0. , 5. );
   TH1F * dR_l_4 = new TH1F("dR_l_4", "dR of loose mu to closest jet", 50 , 0. , 5. );
...
//qier rev or rev2 
      if(rev==1&&(!(((massZ1>60&&massZ1<120)&&(massZ2<60||massZ2>120))||
         ((massZ2>60&&massZ2<120)&&(massZ1<60||massZ1>120))))) continue; 
      if(rev==2&&((massZ1>60&&massZ1<120)||(massZ2>60&&massZ2<120))) continue;
      if( debug ) cout  << "\n Final Z1 properties: "
			<< "\n pxZ1 " << pxZ1
			<< "\n pyZ1 " << pyZ1
			<< "\n pzZ1 " << pzZ1
...
//      if(!(sortedpT[0]>20 && sortedpT[1]>10 && sortedpT[2] >5 && sortedpT[3]>5)) continue;
      //4mu selection qier
      if(channel==1&&(Z1tag!=1||Z2tag!=1)) continue;
      if(channel==2&&(!((Z1tag==1&&Z2tag==2)||(Z1tag==2&&Z2tag==1)))) continue;
      if(channel==3&&(Z1tag!=2||Z2tag!=2)) continue;
...
//qier excluse 40<MET<80
      if(metcut&&RECO_CORMETMUONS>40&&RECO_CORMETMUONS<80) continue;
/*      
      double delta1 = sqrt( pow( DELTAPHI( L1P4.Phi() , L2P4.Phi() ),2) + pow(L1P4.Eta() - L2P4.Eta(),2) );
      double delta2 = sqrt( pow( DELTAPHI( L1P4.Phi() , L3P4.Phi() ),2) + pow(L1P4.Eta() - L3P4.Eta(),2) );
      double delta3 = sqrt( pow( DELTAPHI( L1P4.Phi() , L4P4.Phi() ),2) + pow(L1P4.Eta() - L4P4.Eta(),2) );
      double delta4 = sqrt( pow( DELTAPHI( L2P4.Phi() , L3P4.Phi() ),2) + pow(L2P4.Eta() - L3P4.Eta(),2) );
      double delta5 = sqrt( pow( DELTAPHI( L2P4.Phi() , L4P4.Phi() ),2) + pow(L2P4.Eta() - L4P4.Eta(),2) );
      double delta6 = sqrt( pow( DELTAPHI( L3P4.Phi() , L4P4.Phi() ),2) + pow(L3P4.Eta() - L4P4.Eta(),2) );

      if(delta1<0.2 || delta2<0.2 || delta3<0.2 || delta4<0.2 || delta5<0.2 || delta6<0.2) continue;
*/
...
//      if(ss!=1) continue;
      ...
      hPtLep1_5->Fill(sortedpT[0],newweight);
      hPtLep2_5->Fill(sortedpT[1],newweight);
      hPtLep3_5->Fill(sortedpT[2],newweight);
      hPtLep4_5->Fill(sortedpT[3],newweight);
	...
	if (flagFSR==1){
     ...
     double delta1=5, delta2=5, delta3=5, delta4=5;
...
//qier just for 4 chosen ele
       if(delta1>sqrt( pow( DELTAPHI( L1P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L1P4.Eta() - RECO_PFJET_ETA[i],2) ))
       delta1 = sqrt( pow( DELTAPHI( L1P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L1P4.Eta() - RECO_PFJET_ETA[i],2) );
       
       if(delta2>sqrt( pow( DELTAPHI( L2P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L2P4.Eta() - RECO_PFJET_ETA[i],2) ))
       delta2 = sqrt( pow( DELTAPHI( L2P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L2P4.Eta() - RECO_PFJET_ETA[i],2) );
     
      if(delta3>sqrt( pow( DELTAPHI( L3P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L3P4.Eta() - RECO_PFJET_ETA[i],2) ))
       delta3 = sqrt( pow( DELTAPHI( L3P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L3P4.Eta() - RECO_PFJET_ETA[i],2) );

      if(delta4>sqrt( pow( DELTAPHI( L4P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L4P4.Eta() - RECO_PFJET_ETA[i],2) ))
      delta4 = sqrt( pow( DELTAPHI( L4P4.Phi() , RECO_PFJET_PHI[i] ),2) + pow(L4P4.Eta() - RECO_PFJET_ETA[i],2) );
...
//    	   if (fabs(RECOELE_SIP[iLe[ele]])>=4.) continue;
     ...
     dR_l_1->Fill(delta1,newweight);
     dR_l_2->Fill(delta2,newweight);
     dR_l_3->Fill(delta3,newweight);
     dR_l_4->Fill(delta4,newweight);
