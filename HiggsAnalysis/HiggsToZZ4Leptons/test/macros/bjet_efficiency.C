...
//   TTree *finaltree = fChain->CloneTree(0);
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
	...
	pu_weight=double(puweight->GetBinContent(binx));
      ...
      if(debug) cout << "Starting weight + pileup = " << newweight << endl;
...
// MVA v2 non iso
//        double EB1_10=0.917-1.0355*exp(-RECOELE_PT[i]/2.7382);
//        double EB2_10=0.962-3.1390*exp(-RECOELE_PT[i]/8.7579);
//        double EE_10=0.890-4.3528*exp(-RECOELE_PT[i]/10.124);
//MVA v2 iso
...
//          if(RECOELE_CHARGE[ iLe[j] ] != RECOELE_CHARGE[ iLe[i] ]) continue;
	...
	continue;
      ...
      if(debug) cout << "Number of Z passing the isolation and the 60 < mll < 120 cut is= " << Zcandisolmassvector.size() << endl;
...
//            double sf_hlt = mu_scale_factors_hlt_p1->GetBinContent(binx3,biny3)*19.666/35.812+mu_scale_factors_hlt_p2->GetBinContent(binx32,biny32)*16.146/35.812;
      ...
      if(debug) cout << "Starting weight + pileup + efficiency= " << newweight << endl;
      if(debug) cout << "Efficiency Weight for the 4l: " << eff_weight << " Final weight= " << newweight << endl;
      
      //only keep 2mu (qier) 
      if(Z1tag!=1) continue;

      if(jentry%5000 == 0) cout << "find 1 Z" << endl;
      TLorentzVector Z1P4;
