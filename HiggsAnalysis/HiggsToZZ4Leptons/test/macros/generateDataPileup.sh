...
lumimask="/uscms/home/zwang4/data_lumi/2017/data_BF.json"
#normtag=`python -c "from DevTools.Utilities.utilities import getNormtag; print getNormtag('$runPeriod')"`
pileupjson="/eos/uscms/store/user/wangz/ntuple/pileup/2017/pileup_latest.txt"
mkdir -p pileup
...
for xsec in $central; do
    ...
    echo $xsec
    pileupCalc.py -i $lumimask --inputLumiJSON $pileupjson --calcMode true  --minBiasXsec $xsec --maxPileupBin $maxBins --numPileupBins $maxBins pileup/PileUpData_BF.root
    echo $up
