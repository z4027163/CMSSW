...
//  TH2F *mu_scale_factors_id_p1 = (TH2F*)gDirectory->Get("MC_NUM_MediumID2016_DEN_genTracks_PAR_pt_eta/abseta_pt_ratio");
  ...
  TFile *j_scale_factors3_p1 = new TFile("btag/fake_gsf_BH.root");
  TH2F *j_eff_p1 = (TH2F*)gDirectory->Get("Ele_8");
...
*/
	...
	if (flagFSR==1){
     ...
     if(RECOELE_PT[iLe[0]]<20) continue;
      ...
      for( int i = 0; i < RECO_NELE; ++i ){
        ...
        if( RECOELE_PT[i] > 10. && fabs(RECOELE_ETA[i]) < 2.5 );
          // && RECOELE_gsftrack_expected_inner_hits[i]<=1 ) /* ok */ ;
           ...
           hPtJet_8->Fill(RECOELE_PT[i],newweight);
           hYJet_8->Fill(RECOELE_ETA[i],newweight);

           double Eta=RECOELE_ETA[i];
           if(RECOELE_ETA[i]>2.4) Eta=2.4;
           if(RECOELE_ETA[i]<-2.4) Eta=-2.4;
           cout << "test1"<<endl;
           int biny = j_eff_p1->GetYaxis()->FindBin(RECOELE_PT[i]);
           int binx = j_eff_p1->GetXaxis()->FindBin(Eta);
           cout << "test2" << endl;
        ...
        JET0.SetPtEtaPhiE(RECOELE_PT[i],RECOELE_ETA[i],RECOELE_PHI[i],RECOELE_PT[i]*TMath::CosH(RECOELE_ETA[i]));
