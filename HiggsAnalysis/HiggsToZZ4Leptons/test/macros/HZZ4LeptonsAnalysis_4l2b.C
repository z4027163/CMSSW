...
/*
    ...
    */ 
	...
	iL_loose_mu[i]=-999.;
 	...
 	if( ( RECOMU_isGlobalMu[i] || RECOMU_isTrackerMu[i] )
            && RECOMU_isPFMu[i]
	    && RECOMU_PT[i] > 7. 
	...
	iL_loose_e[i]=-999.;
 	...
 	if( RECOELE_PT[i] > 7. 
	  ...
	  if( 
(RECOMU_isPFMu[mu] || (RECOMU_isTrackerHighPtMu[mu] && RECOMU_PT[mu] > 200.))
	      ...
	      && fabs(RECOMU_SIP[mu])<4. // TightID + SIP cut
           //  RECOMU_isMedium[mu] && fabs(RECOMU_SIP[mu])<4. //MediumID + SIP cut 
	      );
	  else continue;
      ...
      for( int i = 0; i < RECO_NMU; ++i ){
        ...
        if( debug ) cout << "\n Lepton i="<< i <<" properties: "
       	...
       	// Tight muons
 	if( 
(RECOMU_isPFMu[i] || (RECOMU_isTrackerHighPtMu[i] && RECOMU_PT[i] > 200.))
...
//      if(debug) cout << "Efficiency Weight for the Z1: " << eff_weight_3 << " Final weight for Z1= " << newweight << endl;
	...
	if( pow((pTcleanedgoodZ.at(i).massvalue - Zmass),2)+pow((pTcleanedgoodZ.at(j).massvalue - Zmass),2) < pow((massZ1 - Zmass),2)+pow((massZ2 - Zmass),2)){
	  int a,b;
          if(pTcleanedgoodZ.at(i).massvalue > pTcleanedgoodZ.at(j).massvalue){ a=i; b=j;}
          else a=j,b=i;
...
//      hMZ2->Fill( massZ2,newweight );
//      hPtZ2->Fill( ptZ2,newweight );
//      hYZ2->Fill( Y_Z2,newweight );
