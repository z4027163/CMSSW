...
//      if (!(Run==274422 && LumiSection==484 && Event==843958845)) continue;
      ...
      if( RECO_PFJET_N >50) RECO_PFJET_N =50 ;
...
/*
      if (MC_type == "Spring17"&&(datasetName.Contains("ZZTo4L")||datasetName.Contains("GluGluTo4L"))){
       double Ngenjet=0;
       double NG_test=0;
        for(int k=0;k<50;k++){
...
*/ 
   ...
   //    cout << "GenJet number= " << Ngenjet << endl;
       ...
       hfit_pdf_up->Fill(NG_test,newweight);
       hfit_pdf_dow->Fill(Ngenjet,newweight);
       if(njetrw&&(datasetName.Contains("ZZTo4L")||datasetName.Contains("GluGluTo4L"))){
       if(Ngenjet==0) newweight=newweight*1.10;
       else if(Ngenjet==1) newweight=newweight*0.825;
       else if(Ngenjet==2) newweight=newweight*1.10;
       else newweight=newweight*1.15;
	 ...
	 if (jetfail[i]==0){
	   ...
	   njets_pass++;
          ...
          if(!match_jet && MC_type == "Spring17" && DATA_type == "NO") {
           npu_pass++;
           if(RECO_PFJET_PT[i]<45) npu_1_pass++;
           }
       ...
       //      if(cSV_BTagJet_DISCR[i]> 0.5803) {ppu=p_bpu; isf=incb_sf;}
             ppu=p_pu; isf=inc_sf;
             npujetweight=npujetweight*pujet_sf/(1+(isf-1)*ppu);
            }
           }
         ...
         if( MC_type == "Spring17" && DATA_type == "NO"){
            if(RECOBOT_MatchingMCTruth[i+1]==1){
             hPtJet_8_b->Fill(RECO_PFJET_PT[i],newweight*pujet_sf);
            }
            else if(RECOBOT_MatchingMCTruth[i+1]==2){
             hPtJet_8_c->Fill(RECO_PFJET_PT[i],newweight*pujet_sf);
            }
            else if(RECOBOT_MatchingMCTruth[i+1]==3){
             hPtJet_8_l->Fill(RECO_PFJET_PT[i],newweight*pujet_sf);
            }
            else if(RECOBOT_MatchingMCTruth[i+1]==5){
             hPtJet_8_o->Fill(RECO_PFJET_PT[i],newweight*pujet_sf);
            }
           }
