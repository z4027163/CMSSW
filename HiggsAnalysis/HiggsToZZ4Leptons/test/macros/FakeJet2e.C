...
//          if(RECOMU_CHARGE[ iL[j] ] != RECOMU_CHARGE[ iL[i] ]) continue;
      ...
      for(int i = 0; i < RECO_PFJET_N; ++i){
          double Pt0=RECO_PFJET_PT[i];
          double Eta0=RECO_PFJET_ETA[i];
          double Phi0=RECO_PFJET_PHI[i];

          if(RECO_PFJET_PT[i]>10. && fabs(RECO_PFJET_ETA[i])<2.5){
         ...
         for(int k=0.;k<Nphotons;k++) {
           if (iLp_l[k]!=-1 && (iLp_tagEM[k]==0 || iLp_tagEM[k]==1) ) {
             double deltaR = sqrt( pow(DELTAPHI(RECO_PFJET_PHI[i],RECOPFPHOT_PHI[iLp[k]]),2) + pow(RECO_PFJET_ETA[i] - RECOPFPHOT_ETA[iLp[k]],2));
             if (deltaR<0.4){
               jetfail3[i]=1;
               break;
             }
        ...
        if (jetfail2[j]==0&&jetfail3[i]==0){
           ...
           int biny0 = j_eff_p1->GetYaxis()->FindBin(RECO_PFJET_PT[i]);
           int binx0 = j_eff_p1->GetXaxis()->FindBin(RECO_PFJET_ETA[i]);
           double scale_fkj0=j_eff_p1->GetBinContent(binx0,biny0);

           z2weight=scale_fkj*scale_fkj0;
        }
        else continue;
       }   
      else continue;
	  ...
	  float pTphot=-999.;
          Lepton1.SetPtEtaPhiM(RECO_PFJET_PT[i], RECO_PFJET_ETA[i], RECO_PFJET_PHI[i], 0.000511);
...
//trigger SF 
      ...
      if(Z1tag==1&&Z2tag==1){
        ...
        indextwoele[1]=indexleptonfinal[1];
          ...
          leadpt=RECOELE_PT[indexleptonfinal[0]]; leadeta=RECOELE_ETA[indexleptonfinal[0]];indextwoele[0]=indexleptonfinal[0];
          subpt=RECOELE_PT[indexleptonfinal[1]];subeta= RECOELE_ETA[indexleptonfinal[1]];indextwoele[1]=indexleptonfinal[1];
        }
    ...
    for(int i=0; i<2; i++){
     ...
     if(debug )cout << "eff_weight" << eff_weight << endl;
      ...
      for(int i = 2; i < 4; ++i){
        ...
        if (Z2tag==2) {tmp_pt[i] =  RECO_PFJET_PT[indexleptonfinal[i]]; tmp_type[i]=2;}
 ...
 */
	...
	if (flagFSR==1){
     ...
     for(int i=0;i<RECO_PFJET_N;i++){
          ...
          if(RECO_PFJET_PT[i]<-100) continue;
          if(i==indexlep2Z2||i==indexlep1Z2) continue;
