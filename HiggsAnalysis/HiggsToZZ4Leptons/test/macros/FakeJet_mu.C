...
//  TGraph *mu_scale_factors_tk = (TGraph*)gDirectory->Get("ratio_eff_eta3_dr030e030_corr");
 ...
 TH1F * hDPHI_JET_MET_8 = new TH1F("hDPHI_JET_MET_8", "polar angle between jet and E_{T,miss}", 3150, 0., 3.15 );
   ...
   TH1F * hPtRatio_j = new TH1F("hPtRatio_j", "Pt of bot after selection step 5", 100 ,  0 , 5. );
   ...
   TH1F * hPtRatio = new TH1F("hPtRatio", "Pt of bot after selection step 5", 100 ,  0 , 5. );

   TH1F * hPtBot_9 = new TH1F("hPtBot_9", "Pt of bot after selection step 5", 300 ,  0 , 600 );
   hPtBot_9->SetXTitle("pt_bot  (GeV)");
...
//          if(RECOELE_CHARGE[ iLe[j] ] != RECOELE_CHARGE[ iLe[i] ]) continue;
	...
	if (!(Zcandvector.at(index).massvalue > 80 && Zcandvector.at(index).massvalue < 100)) continue;
...
//	   if (fabs(RECOMU_SIP[iL[mu]])>=4.) continue;  //qier
           ...
           if(iL[mu]!=indexleptonfinal[0]&&iL[mu]!=indexleptonfinal[1]) continue;
...
//qier b-tag sf                       else if(datasetName.Contains("TT")) sf_l=b_eff_tt_p5->GetBinContent(binxb);
    ...
    if(njets_pass<1) continue;
    hPtJet_8->Fill(JET1.Pt(),newweight);
    int jindex=-1;
    for(int mu = 0; mu < N_good; ++mu){
       if (RECOMU_PFX_dB_new[iL[mu]]>=0.20) continue;
       if(iL[mu]==indexleptonfinal[0]||iL[mu]==indexleptonfinal[1]) continue;
           double deltaR = sqrt( pow(DELTAPHI(RECO_PFJET_PHI[jet1],RECOMU_PHI[iL[mu]]),2) + pow(RECO_PFJET_ETA[jet1] - RECOMU_ETA[iL[mu]],2));
           if(debug) cout << "1st lepton muon: " << " pT=" << RECOMU_PT[iL[mu]] <<" deltaR "<< deltaR <<endl;
           if (deltaR<0.15){
            hPtJet_9->Fill(JET1.Pt(),newweight);
            hPtJet_7->Fill(RECOMU_PT[iL[mu]],newweight);
            hPtRatio_j->Fill(RECOMU_PT[iL[mu]]/JET1.Pt());
            jindex=iL[mu];
             break;
           }
         }
...
//     hDPHI_MAX_JET_MET_8->Fill(max_dphi_jet_met, newweight); 
    ...
    hPtBot_8->Fill(BOT1.Pt(),newweight);
    bool mumatch=false;
    int muindex=-1;
    for(int mu = 0; mu < N_good; ++mu){
       if (RECOMU_PFX_dB_new[iL[mu]]>=0.20) continue;
       if(iL[mu]==indexleptonfinal[0]||iL[mu]==indexleptonfinal[1]) continue;
           double deltaR = sqrt( pow(DELTAPHI(RECO_PFJET_PHI[bot1],RECOMU_PHI[iL[mu]]),2) + pow(RECO_PFJET_ETA[bot1] - RECOMU_ETA[iL[mu]],2));
           if(debug) cout << "1st lepton muon: " << " pT=" << RECOMU_PT[iL[mu]] <<" deltaR "<< deltaR <<endl;
           if (deltaR<0.15){
            hPtBot_9->Fill(BOT1.Pt(),newweight);
            hPtBot_7->Fill(RECOMU_PT[iL[mu]],newweight);
            hPtRatio->Fill(RECOMU_PT[iL[mu]]/BOT1.Pt());
            muindex=iL[mu];
            mumatch=true;
             break;
           }
...
//     double btag_weight=1;
     ...
     f_lumi = LumiSection;
       ...
       f_lept1_pt  = RECOMU_PT[muindex] ;
       f_lept1_eta = RECOMU_ETA[muindex] ;
       f_lept1_phi = RECOMU_PHI[muindex];
       //    f_lept1_mvaid = RECOMU_mvaNonTrigV0[indexleptonfinal[0]];
       f_lept2_pt  = RECO_PFJET_PT[bot1];
       f_lept2_eta = RECO_PFJET_ETA[bot1];
       f_lept2_phi = RECO_PFJET_PHI[bot1];
