...
HZZ4LeptonsZZMassErrors::HZZ4LeptonsZZMassErrors(const edm::EventSetup& eventSetup) {
  eventSetup.get<IdealMagneticFieldRecord>().get(magfield);  
}


double HZZ4LeptonsZZMassErrors::pError(const pat::Electron* ele) {
  double dp=0;
  if (! ele->ecalDriven()) {
...
float HZZ4LeptonsZZMassErrors::massError(const reco::Candidate* Z1Lp, 
